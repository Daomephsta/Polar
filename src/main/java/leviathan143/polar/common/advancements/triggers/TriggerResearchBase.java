...
public abstract class TriggerResearchBase<T extends ICriterionInstance> extends TriggerBase<T>
{
	...
	{
		Predicate<Listener<T>> hasParent = l -> player.getAdvancements().getProgress(getAdvancement(l).getParent()).isDone();
		super.grantPassedCriteria(player, and(hasParent, conditions));
	}
	
	private Advancement getAdvancement(Listener<T> listener)
	{
		try
		{
			return (Advancement) Listener_advancement.get(listener);
		}
		catch (IllegalArgumentException | IllegalAccessException e)
		{
			throw new RuntimeException("Could not access listener advancement", e);
		}
	}
}
