package leviathan143.polar.common.handlers.research;

import java.util.*;
import java.util.stream.Collectors;

import leviathan143.polar.common.advancements.triggers.TriggerRegistry;
import leviathan143.polar.common.handlers.research.ObserveFallingBlockHandler.FallingBlockTracker.Status;
...
public class ObserveFallingBlockHandler
{
	...
	public static void trackFallingBlock(EntityFallingBlock fallingBlock, EntityPlayerMP entityPlayer)
	{
		World world = fallingBlock.getEntityWorld();
		if (world.isRemote)
			return;
		if (TriggerRegistry.OBSERVE_FALLING_BLOCK.hasListeners(entityPlayer.getAdvancements()))
	...
	{
		return trackedFallingBlocks.computeIfAbsent(world, k -> Collections.newSetFromMap(new WeakHashMap<>()));
	}

	public static void tick(World world, Phase phase)
	{
		...
		for (Iterator<FallingBlockTracker> iter = notFalling.iterator(); iter.hasNext();)
		{
			FallingBlockTracker tracker = iter.next();
			if (tracker.getStatus() == Status.HIT_GROUND 
				&& tracker.fallingBlock.getOrigin().distanceSq(tracker.fallingBlock.getPosition()) >= 32 * 32)
			{
	...
	public static class FallingBlockTracker
	{
		private final EntityPlayerMP entityPlayer;
		private final EntityFallingBlock fallingBlock;
		
		public FallingBlockTracker(EntityPlayerMP entityPlayer, EntityFallingBlock fallingBlock)
		{
		...
		public Status getStatus()
		{
			...
			if (!fallingBlock.isAddedToWorld())
